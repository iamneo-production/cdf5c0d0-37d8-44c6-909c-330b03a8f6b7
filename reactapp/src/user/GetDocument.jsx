...
export const GetDocument = () => {
  ...
  const [documentUpload, setDoc] = useState(null);
  ...
  const addDocument = async (event) => {
    ...
    const bytes = await convertFileToBytes(documentUpload);
  
    const formData = new FormData();
    formData.append('documentId', sessionStorage.getItem("loanId"));
    formData.append('documentType', documentType);
    formData.append('documentUpload', new Blob([bytes]));
  
  
    try {
      await axios.post("https://8080-babebfdaaddaacebcaffeabdcdceeeeaf.project.examly.io/user/getDocuments", formData, {
        headers: {
      ...
      <div className="form-control mt-4 px-5 bg-primary bg-opacity-10">
        ...
        <form onSubmit={addDocument}>
          ...
          <div className="d-flex py-2 gap-3">
            <button type="button" onClick={validateSelectedFile} className="btn btn-primary">
              Upload Documents
            </button>
            {isSuccess && <p className="success-message text-success fw-bold">Upload successful</p>}
            {errorMsg && <p className="error-message text-danger fw-bold">{errorMsg}</p>}
          </div>
          <div className="d-grid py-2">
            {isSuccess && <button type="submit" className="btn btn-success">Add Documents</button>}
          </div>
        </form>
      </div>
    </div>
